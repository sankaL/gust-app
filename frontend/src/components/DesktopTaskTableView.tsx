...
function TaskDueCell({ task, onMove }: { task: TaskSummary; onMove?: Actions['onMoveDueDate'] }) {
  if (task.status === 'open' && onMove) return <td className="px-4 py-3"><input type="date" value={task.due_date ?? ''} onChange={(event) => onMove(task, event.target.value || null)} className="w-36 rounded-card bg-surface-dim px-2 py-1.5 text-sm" aria-label={`Move ${task.title} due date`} /></td>
...
function TaskRow({ task, status, actions }: { task: TaskSummary; status: string; actions: Actions }) {
