...
function ScheduleFields({ draft, originalDueDate, groups, open, setOpen, update, disabled }: { draft: TaskDetailDraft; originalDueDate: string | null; groups: GroupSummary[]; open: boolean; setOpen: (value: boolean) => void; update: DraftUpdater; disabled: boolean }) {
  ...
  return <div className="rounded-card bg-surface/35">
    ...
    <div className="grid border-b border-white/10 px-4 py-3 sm:grid-cols-[10rem_minmax(0,1fr)]"><p className="text-xs uppercase">Due date</p><DatePicker value={draft.dueDate || null} mode="date" disabled={disabled} placeholder="Select a date" onChange={(value) => update((current) => value ? shouldShiftReminderForDueDate(originalDueDate, value) ? { ...current, dueDate: value, reminderAt: current.reminderAt ? shiftReminderDateTimeLocal(current.reminderAt, value) : '', reminderDate: current.reminderDate ? value : '' } : { ...current, dueDate: value } : { ...current, dueDate: '', reminderAt: '', reminderDate: '', recurrence: null })} /></div>
...
export function TaskDetailFields({ draft, originalDueDate, groups, groupOpen, setGroupOpen, update, disabled }: { draft: TaskDetailDraft; originalDueDate: string | null; groups: GroupSummary[]; groupOpen: boolean; setGroupOpen: (value: boolean) => void; update: DraftUpdater; disabled: boolean }) {
  return <section className="space-y-5"><label className="block text-[0.68rem] uppercase text-on-surface-variant">Title<input value={draft.title} onChange={(event) => update((current) => ({ ...current, title: event.target.value }))} className="mt-2 w-full rounded-card bg-surface/65 px-4 py-3 font-display text-2xl" disabled={disabled} aria-label="Task title" /></label><label className="block text-[0.68rem] uppercase text-on-surface-variant">Context<textarea value={draft.description} onChange={(event) => update((current) => ({ ...current, description: event.target.value }))} rows={7} className="mt-2 w-full resize-none rounded-card bg-surface/55 px-4 py-3 text-sm" disabled={disabled} aria-label="Task description" /></label><ScheduleFields draft={draft} originalDueDate={originalDueDate} groups={groups} open={groupOpen} setOpen={setGroupOpen} update={update} disabled={disabled} /></section>
}
