...
import { useSessionTimezoneSync } from '../hooks/useSessionTimezoneSync'
import {
...
function setup(session: SessionStatus) {
  ...
  const hook = renderHook(() => {
    ...
    return useSessionTimezoneSync(sessionQuery.data)
  }, { wrapper })
  return { client, hook }
}
...
describe('session timezone synchronization', () => {
  it('persists a different browser timezone before marking task dates ready', async () => {
    ...
    expect(mockedUpdateSessionTimezone).toHaveBeenCalledWith('America/Toronto', 'csrf-token')
  ...
  it('rechecks once when the app returns to the foreground after a device timezone change', async () => {
    ...
    await waitFor(() => expect(hook.result.current.isReady).toBe(true))
    expect(mockedUpdateSessionTimezone).toHaveBeenCalledTimes(1)
    expect(mockedUpdateSessionTimezone).toHaveBeenCalledWith('America/Vancouver', 'csrf-token')
  })
})
