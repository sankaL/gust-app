...
export function CaptureStaging(props: StagingProps) {
  return <div className="mt-4 space-y-4">{props.processing ? <div className="rounded-card bg-[rgba(22,22,22,0.94)] p-4"><h3 className="font-display text-xl text-on-surface">Organizing your tasks</h3><p className="mt-2 text-sm text-on-surface-variant">Extracted tasks are still arriving.</p></div> : null}<StagingTable tasks={props.tasks} isLoading={props.isLoading} title={props.title} subtext={props.subtext} onApprove={props.onApprove} onDiscard={props.onDiscard} onApproveAll={props.onApproveAll} onDiscardAll={props.onDiscardAll} onTaskClick={props.onTaskClick} emptyMessage="No newly captured tasks to review" />{props.onDone ? <button type="button" onClick={props.onDone} className="rounded-pill border border-outline px-3 py-1.5 text-sm text-on-surface-variant">Done</button> : null}</div>
}
