...
function Toggle({ label, checked, disabled, onChange }: { label: string; checked: boolean; disabled?: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex min-h-11 cursor-pointer items-center justify-between gap-4 rounded-soft bg-surface-container-high px-4 py-2.5 font-body text-sm font-medium text-on-surface transition-colors duration-150 hover:bg-surface-bright focus-within:ring-2 focus-within:ring-primary/70 has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-60"><span>{label}</span><span className="relative inline-flex h-6 w-10 shrink-0" aria-hidden="true"><input aria-label={label} role="switch" type="checkbox" className="peer sr-only" checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} /><span className="absolute inset-0 rounded-full bg-outline/45 transition-colors duration-150 peer-checked:bg-primary peer-disabled:bg-surface-container-highest" /><span className="absolute left-1 top-1 h-4 w-4 rounded-full bg-surface shadow-sm transition-transform duration-150 peer-checked:translate-x-4" /></span></label>
}
...
export function SettingsRoute() {
  ...
  const save = useMutation({
    ...
    onSuccess: (next) => queryClient.setQueryData(['notification-settings'], next),
