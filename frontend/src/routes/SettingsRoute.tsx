...
export function SettingsRoute() {
  ...
  useEffect(() => {
    ...
    void refetchSettings()
      .then((result) => {
        if (result.isError) {
          notifyError('Pushover connected, but Gust could not refresh your settings. Try again.')
          return
        }
  ...
  }, [location.pathname, location.search, navigate, notifyError, notifySuccess, refetchSettings])
  ...
  if (!settings) {
    if (settingsQuery.isError) {
      return <section className="space-y-3 py-8" role="alert"><p className="font-body text-sm text-on-surface-variant">Could not refresh notification settings. Your Pushover connection may be saved, but Gust cannot confirm its current state.</p><button type="button" onClick={() => void refetchSettings()} disabled={settingsQuery.isFetching} className="min-h-11 rounded-pill bg-primary px-4 font-body text-sm font-medium text-surface disabled:opacity-60">{settingsQuery.isFetching ? 'Retrying…' : 'Try again'}</button></section>
    }
    return <section className="py-8" aria-busy="true"><p className="font-body text-sm text-on-surface-variant">Loading settings…</p></section>
  }
  const connectedAndEnabled = settings.pushover_connected && settings.pushover_enabled
  return <section className="mx-auto max-w-2xl space-y-6 pb-8">{settingsQuery.isError ? <section className="space-y-3 rounded-card bg-surface-container-high p-4" role="alert"><p className="font-body text-sm text-on-surface-variant">Could not refresh notification settings. Some information may be out of date.</p><button type="button" onClick={() => void refetchSettings()} disabled={settingsQuery.isFetching} className="min-h-11 rounded-pill bg-primary px-4 font-body text-sm font-medium text-surface disabled:opacity-60">{settingsQuery.isFetching ? 'Retrying…' : 'Try again'}</button></section> : null}<header><p className="font-body text-xs font-semibold uppercase tracking-[0.16em] text-primary">Account</p><h1 className="font-display text-3xl text-on-surface">Notification settings</h1><p className="mt-2 font-body text-sm leading-6 text-on-surface-variant">Digest schedules use Eastern time. Your date-only reminders use {settings.timezone}.</p></header>
