...
function Toggle({ label, checked, disabled, onChange }: { label: string; checked: boolean; disabled?: boolean; onChange: (checked: boolean) => void }) {
  return <label className="flex min-h-11 cursor-pointer items-center justify-between gap-4 rounded-soft bg-surface-container px-4 py-3 text-sm text-on-surface transition hover:bg-surface-container-highest has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-60"><span>{label}</span><span className="relative inline-flex h-6 w-11 shrink-0"><input aria-label={label} role="switch" type="checkbox" className="peer sr-only" checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} /><span aria-hidden="true" className="absolute inset-0 rounded-full bg-surface-container-highest transition peer-checked:bg-primary peer-focus-visible:ring-2 peer-focus-visible:ring-primary peer-disabled:opacity-50" /><span aria-hidden="true" className="absolute left-1 top-1 h-4 w-4 rounded-full bg-on-surface transition peer-checked:translate-x-5 peer-checked:bg-surface" /></span></label>
}
