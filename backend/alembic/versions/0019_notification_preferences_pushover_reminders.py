...
Revision ID: 0019_pushover_reminders
...
revision: str = "0019_pushover_reminders"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "email_daily_enabled", sa.Boolean(), nullable=False, server_default=sa.text("true")
        ),
        sa.Column(
            "email_weekly_enabled", sa.Boolean(), nullable=False, server_default=sa.text("true")
        ),
        sa.Column(
            "pushover_enabled", sa.Boolean(), nullable=False, server_default=sa.text("false")
        ),
        sa.Column(
            "pushover_task_reminders_enabled",
            sa.Boolean(),
            nullable=False,
            server_default=sa.text("false"),
        ),
        sa.Column(
            "pushover_daily_digest_enabled",
            sa.Boolean(),
            nullable=False,
            server_default=sa.text("false"),
        ),
        sa.Column(
            "pushover_weekly_digest_enabled",
            sa.Boolean(),
            nullable=False,
            server_default=sa.text("false"),
        ),
    ...
    op.create_index(
        "ix_reminders_status_next_attempt_at",
        "reminders",
    ...
    with op.batch_alter_table("digest_dispatches", schema=schema) as batch:
        batch.add_column(
            sa.Column(
                "channel", sa.String(length=16), nullable=False, server_default=sa.text("'email'")
            )
        )
