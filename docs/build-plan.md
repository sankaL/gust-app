...
| ADH-15 | Provision production Railway, Supabase, Resend, and digest cron infrastructure | IN PROGRESS | 2026-03-26 | Fresh Supabase project created and linked, Alembic upgraded to `0008_digest_dispatches`, `gustapp.ca` and `api.gustapp.ca` are live on Railway, daily/weekly cron services are deployed, manual digest runs succeed, and hosted Supabase Google auth is configured. Remaining blocker: `auth.gustapp.ca` still requires the Supabase Custom Domain add-on. | Tech, Runbook, AGENTS |
