...
| BUG-17 | Digest emails skip open tasks without due dates | IN PROGRESS | 2026-03-27 | Backend now includes up to 5 undated open tasks in daily/weekly digests, treats undated-only periods as sendable, refreshes the branded email template with logo + web CTA, and adds regression coverage. Remaining verification blocker: live Railway cron/env/log checks are still blocked until CLI auth is restored. | PRD, Tech, AGENTS |
