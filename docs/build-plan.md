...
| ADH-26 | Add Supabase-backed Google auth email allowlist enforcement | DONE | 2026-03-28 | Added `public.allowed_users`, a Supabase `before_user_created` auth hook, backend callback/session allowlist enforcement for already-created users, login-page denial messaging, local-runtime Supabase migration syncing, auth regression coverage updates, and follow-up fixes for blocked-session cookie clearing plus production runtime-role table grants. | PRD, Schema, Runbook, Tech, AGENTS |
